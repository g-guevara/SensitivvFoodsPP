...
export class ApiService {
  ...
  static async getIngredientReactions() {
    ...
    static async searchProducts(query: string) {
      try {
        // Determinar si el query es un código de barras (solo números)
        const isBarcode = /^\d+$/.test(query);
        
        if (isBarcode) {
          try {
            const product = await this.getProductByBarcode(query);
            return product ? [product] : [];
          } catch (error) {
            // Si no encuentra por código de barras, continúa con búsqueda normal
            console.log('No product found by barcode, trying general search');
          }
        }

        // Búsqueda general con más resultados y campos
        const response = await fetch(
          `${this.OPEN_FOOD_FACTS_API}/search.json?search_terms=${encodeURIComponent(query)}&page_size=100&search_simple=1&fields=code,product_name,brands,ingredients_text,image_url,ingredients`
        );
    ...
    private static mapOpenFoodFactsProduct(product: any) {
